...
if(isset($_POST['order'])){
    ...
    $f_date=mysqli_real_escape_string($conn, $_POST['from']);
    $t_date=mysqli_real_escape_string($conn, $_POST['to']);
    ...
    if(mysqli_num_rows($cart_query) > 0){
        while($cart_item = mysqli_fetch_assoc($cart_query)){
            ...
            $select_event_planner = mysqli_query($conn, "SELECT * FROM `items` WHERE id='$event_product_id' ") or die('query failed-37');
    ...
    if($cart_total == 0){
        ...
        mysqli_query($conn, "INSERT INTO `orders`(user_id, name, number, email, method, address,f_date,t_date, total_products, total_price, placed_on,planner_id,pid) VALUES('$user_id', '$name', '$number', '$email', '$method', '$address','$f_date','$t_date', '$cart_products', '$cart_total', '$placed_on','$planner_id','$event_product_id')") or die(mysqli_error($conn));
...
<section class="before-checkout" style="margin-top:2rem">
    ...
    <div class="flex">
        <div class="check">
            ...
            <a href="#checkout" class="op-btn">Confirm and pay</a>
                ...
                <?php
                    ...
                    if(mysqli_num_rows($select_cart) > 0){
                        while($fetch_cart = mysqli_fetch_assoc($select_cart)){
                            ...
                            $select_event = mysqli_query($conn, "SELECT * FROM `items` WHERE id='$event_product_id'") or die('query failed');
                            ...
                            $event_planner_image=$fetch_user['img'];
...
<section class="checkout" id="checkout">
    ...
    <form action="" method="POST">
        ...
        <div class="flex">
            ...
            <div class="inputBox">
                <span>From :</span>
                <input type="date" required name="from" placeholder="From date">
            </div>
            <div class="inputBox">
                <span>To :</span>
                <input type="date" required name="to" placeholder="To date">
            </div>
            
        </div>

        <center><input type="submit" name="order" value="Order Now" class="op-btn" ></center>
